...
$path=realpath("image.php");
$path=str_replace("\image.php", "", $path);
$path=str_replace("/image.php", "", $path);
define ('LOCAL_FILE_BASE_DIRECTORY', $path);
...
class timthumb {
	...
	public function __construct(){
		...
		if(BLOCK_EXTERNAL_LEECHERS && array_key_exists('HTTP_REFERER', $_SERVER) && (! preg_match('/^https?:\/\/(?:www\.)?' . $this->myHost . '(?:$|\/)/i', $_SERVER['HTTP_REFERER']))){
			...
			header ('Expires: "access plus 1 month"');
			echo $imgData;
			return false;
			exit(0);
		}
	...
	protected function param($property, $default = ''){
		if (isset ($_GET[$property])) {
			...
			return $wp_query->query_vars[$property];
		} else {
			return $default;
		}
	}
